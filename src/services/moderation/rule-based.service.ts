...
const TOXIC_BLOCKLIST_NORMALIZED: readonly string[] = TOXIC_BLOCKLIST.map(
  (phrase) => normalizeForBlocklist(phrase)
);
  ...
  checkContent(text: string): RuleCheckResult {
    try {
      ...
      return {
        ...
        reason:
          spamReasons.length > 0 ? spamReasons.join(",") : undefined,
      };
    } catch {
      return {
        isClearViolation: false,
