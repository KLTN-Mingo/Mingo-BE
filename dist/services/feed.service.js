...
async function loadPostRelationsForFeedBatch(posts, currentUserId) {
    ...
    const [authors, mediaRows, hashtagRows, mentionRows, likeRows, savedRows] = await Promise.all([
        ...
        like_model_1.LikeModel.find({
            ...
            .select("postId")
            .lean(),
        saved_post_model_1.SavedPostModel.find({
            postId: { $in: postIds },
            userId: currentUserObjectId,
        })
            .select("postId")
    ...
    const savedPostIds = new Set(savedRows.map((r) => r.postId?.toString()).filter(Boolean));
...
exports.FeedService = {
    ...
    async getFriendsFeed(userId, page, limit) {
        ...
        const rawCandidates = await post_model_1.PostModel.find({
            ...
            isHidden: false,
            moderationStatus: {
                $in: [post_model_1.ModerationStatus.APPROVED, post_model_1.ModerationStatus.PENDING],
            },
