 ...
 *
 * Run này ÉP 100% mẫu vào Tier 2 bằng context={ reportCount: 1 }.
 * Dùng để đánh giá năng lực thật của specialized model, bất kể cấu hình production.
 *
 ...
 *       npx ts-node scripts/eval-e2e-reported.ts --limit 50
 *       npx ts-node scripts/eval-e2e-reported.ts   (chạy full sau khi verify limit 50 ổn)
 */
...
// Ép 100% mẫu vào Tier 2 — reportCount > 0 bypasses shouldCallAI() random gate
const SIMULATED_CONTEXT = { reportCount: 1 };
...
async function main() {
  ...
  for (let i = 0; i < data.length; i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.all(
      batch.map(async (item) => {
        try {
          const result = await ModerationService.moderateContent(item.text, SIMULATED_CONTEXT);
          return {
            text: item.text,
  ...
  const outPath = path.join(__dirname, "..", "predictions_reported.json");
